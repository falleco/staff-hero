...
import type { GameSession, UserAnalytics } from '@/types/analytics';
import {
  addGameSession as addGameSessionAPI,
  clearUserAnalytics as clearUserAnalyticsAPI,
  getRecentSessions as getRecentSessionsAPI,
...
export function useAnalytics(): UseAnalyticsReturn {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const fetchedAnalytics = await getUserAnalyticsAPI(user.id);
  ...
  const addSession = useCallback(
    async (session: Omit<GameSession, 'id' | 'date'>): Promise<string> => {
      ...
      try {
        ...
        const sessionId = await addGameSessionAPI(user.id, session);
  ...
  const getRecentSessions = useCallback(
    async (limit = 20): Promise<GameSession[]> => {
      ...
      try {
        return await getRecentSessionsAPI(user.id, limit);
      } catch (error) {
  ...
  const getUserAchievements = useCallback(async () => {
    ...
    try {
      return await getUserAchievementsAPI(user.id);
    } catch (error) {
  ...
  const unlockAchievement = useCallback(
    async (achievementId: string): Promise<boolean> => {
      ...
      try {
        const unlocked = await unlockAchievementAPI(user.id, achievementId);
  ...
  const clearData = useCallback(async () => {
    ...
    try {
      await clearUserAnalyticsAPI(user.id);
