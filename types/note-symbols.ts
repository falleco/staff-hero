...
export function getNoteSymbolsForDifficulty(difficulty: 'beginner' | 'intermediate' | 'advanced'): NoteSymbol[] {
  const symbols = Object.values(NOTE_SYMBOLS);
  
  switch (difficulty) {
    case 'beginner':
      return symbols.filter(s => s.difficulty === 'beginner');
    case 'intermediate':
      return symbols.filter(s => s.difficulty === 'beginner' || s.difficulty === 'intermediate');
    case 'advanced':
      return symbols; // All symbols
    default:
      return symbols.filter(s => s.difficulty === 'beginner');
  }
}
