...
import { lineSpacing } from '~/shared/constants/config';
import type { NoteAnimation, NoteState } from '~/shared/types/music';
...
/**
 * Enhanced SemibreveNote component with controlled animations and state management
 *
 * Supports different note states (idle, correct, incorrect) and controlled animations
 * for creation, destruction, and feedback. Integrates with game mechanics for
 * visual feedback and note lifecycle management.
 *
 * @param x - X position on the staff
 * @param y - Y position on the staff
 * @param scale - Scale factor for the note
 * @param color - Base note color
 * @param state - Current note state (idle, correct, incorrect, etc.)
...
export const SemibreveNote = ({
  ...
  useEffect(() => {
    ...
    switch (animation.type) {
      ...
      case NoteAnimationType.DESTRUCTION:
        // Note destruction with feedback
        if (animation.isCorrect !== undefined) {
          // Set color based on correctness
          setCurrentColor(animation.isCorrect ? '#10B981' : '#EF4444'); // Green or Red

          // Show and play destruction animation
      ...
      case NoteAnimationType.FEEDBACK:
        // Feedback animation without destruction
        if (animation.isCorrect !== undefined) {
          const feedbackColor = animation.isCorrect ? '#10B981' : '#EF4444';
          setCurrentColor(feedbackColor);
  ...
  }, [animation, color, noteId, onAnimationComplete]);
  ...
  useEffect(() => {
    switch (state) {
      ...
      case State.HIGHLIGHTED:
        setCurrentColor('#3B82F6'); // Blue highlight
        break;
      case State.CORRECT:
  ...
  const handleAnimationFinish = () => {
    if (animation?.type === NoteAnimationType.DESTRUCTION) {
      setIsVisible(false);
    }
    setShowAnimation(false);
      ...
      {showAnimation && (
        <LottieView
          ...
          style={{
            width: 40,
            height: 40,
            transform: [{ translateX: x - 20 }, { translateY: y - 15 }],
            zIndex: 10,
          }}
          source={
            (animation?.isCorrect ?? true)
              ? require('@/assets/animations/explosion.json') // Green explosion for correct
              : require('@/assets/animations/explosion-red.json') // Red explosion for incorrect
          }
          onAnimationFinish={handleAnimationFinish}
        />
      )}

      {/* SVG Note - main visual */}
      <G
        transform={`translate(${x - 27}, ${y - 5}) scale(0.035, 0.035)`}
        opacity={state === State.DESTROYING ? 0.5 : 1}
      >
        <Path
